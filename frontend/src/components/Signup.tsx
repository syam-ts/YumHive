...
const SignUp = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try{
      ...
      const res = await fetch('/localhost:3600/user/signup', {
        method: 'POST',
        headers: {
  ...
  <div className="flex w-full flex-col md:w-1/2" >
    <div className="lg:w-[28rem] mx-auto my-auto flex flex-col justify-center pt-8 md:justify-start md:px-5 md:pt-0">
      ...
      >
        ...
        <div className="flex flex-col pt-4">
          <div className="focus-within:border-b-gray-500 relative flex overflow-hidden border-b-2 transition">
            <input type="text" id="username" className="w-full flex-1 appearance-none border-gray-300 bg-white px-4 py-2 text-base text-gray-700 placeholder-gray-400 focus:outline-none" placeholder="Username" 
                 onChange={handleChange}
            />
          </div>
        </div>
